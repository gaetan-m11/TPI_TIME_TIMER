...
//U8G2_SSD1322_NHD_256X64_F_4W_SW_SPI u8g2(U8G2_R0, /* clock=*/ 6, /* data=*/ 7, /* cs=*/ 10, /* dc=*/ 9, /* reset=*/ 8); // Enable U8G2_16BIT in u8g2.h
U8G2_SSD1322_NHD_256X64_F_4W_HW_SPI u8g2(U8G2_R0, 19, 4);


// Array of all bitmaps for convenience. (Total uint8_ts used to store images in PROGMEM = 14784)
// Array of all bitmaps for convenience. (Total uint8_ts used to store images in PROGMEM = 10304)
...
char menu_items [NUM_ITEMS] [MAX_ITEM_LENGTH] = {  // array with item names
 ...
 char alerts_item [NUM_ITEMS] [MAX_ITEM_LENGTH] = {  // array with item names
  { "1 minutes" }, 
  { "2 minutes" }, 
  { "5 minutes" }, 
  { "10 minutes" }, 
 ...
 char preset_options [2] [MAX_ITEM_LENGTH] = {
  {"Save"},
  {"Load"}
 };
...
void update_names(String name, int index)
{
  ...
  if (index >= 0 && index < NUM_ITEMS) {
    // Copie du nom dans le tableau à l'index spécifié
...
{
  u8g2.begin();
  u8g2.clearDisplay(); // clear the internal memory
}
...
void battery_display(uint8_t bat_val)
{
  uint8_t x = 228;
  uint8_t y = 0;
  // Effacer uniquement la zone de la batterie
  u8g2.setDrawColor(0); // Définir la couleur de dessin sur noir
  u8g2.drawBox(x, y, bat_logo_width, bat_logo_height); // Dessiner un rectangle noir
  u8g2.setDrawColor(1); // Remettre la couleur de dessin sur blanc

  // Afficher l'icône de la batterie en haut à droite
  u8g2.drawXBMP(x, y, bat_logo_width, bat_logo_height, bat_bitmap);

  // Dessiner la jauge de la batterie à l'intérieur de l'icône
  int gauge_width = map(bat_val, 0, 100, 0, 19); // Mappe le pourcentage de batterie à la largeur de la jauge

  // Dessiner le remplissage de la jauge
  u8g2.drawRBox(x + 4, y + 2, gauge_width, 10, 2); 

  u8g2.updateDisplay(); // Envoyer le tampon à l'affichage
...
void battery_menu_display(uint8_t bat_val)
{
  u8g2.clearDisplay();
  u8g2.setDrawColor(1);
  // Calcul de la largeur du niveau de charge en fonction du pourcentage
  int fillWidth = map(bat_val, 0, 100, 0, 252);

  // Dessine le niveau de charge de la batterie
  u8g2.drawXBM(0, 0, 256, 64, big_bat_logo);

  // Affiche le pourcentage au centre du logo de la batterie
...
{
  // Effacer seulement les parties qui changent
  u8g2.setDrawColor(0);
  u8g2.setFont(u8g2_font_fub35_tr); // Choisir la police pour l'heure
  u8g2.drawBox(0, 0, 226, 50); // Effacer la zone de l'heure
  u8g2.setDrawColor(1);

  // Dessiner l'heure en grand
  char timeStr[9]; // HH:MM:SS
  sprintf(timeStr, "%02d:%02d:%02d", heure, minute, seconde);
  uint8_t w1;
  w1 = u8g2.getStrWidth(timeStr); // Get the text width
  uint8_t centerX1 = (256 - w1) / 2; // Calculate the starting x position to center the text
  u8g2.drawStr(centerX1, 40, timeStr); // Positionner l'heure
  
  // Effacer seulement les parties qui changent
  u8g2.setDrawColor(0);
  u8g2.setFont(u8g2_font_fur11_tr); // Choisir la police pour la date
  u8g2.drawBox(0, 50, 256, 20); // Effacer la zone de la date
  u8g2.setDrawColor(1);

  // Dessiner la date en plus petit
  char dateStr[11]; // JJ/MM/AAAA
  sprintf(dateStr, "%02d/%02d/%04d", jour, mois, 2000 + annee); // Ajouter 2000 à l'année pour obtenir le format AAAA
  uint8_t w2;
  w2 = u8g2.getStrWidth(dateStr); // Get the text width
  uint8_t centerX2 = (256 - w2) / 2; // Calculate the starting x position to center the text
  u8g2.drawStr(centerX2, 60, dateStr); // Positionner la date
  
  // Envoyer le contenu du buffer à l'affichage
  u8g2.updateDisplay();
}
